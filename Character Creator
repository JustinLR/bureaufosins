...
def update_window_color():
    ...
    if bg_color and fg_color:
        update_widget_colors(root, bg_color, fg_color)

def reset_window_color():
    update_widget_colors(root, DEFAULT_BG_COLOR, DEFAULT_FG_COLOR)
...
def setup_menu():
    ...
    file_menu.add_command(label="Export to PDF", command=on_submit)
    file_menu.add_command(label="Close", command=close_application)
    menubar.add_cascade(label="File", menu=file_menu)

    # Create an 'Options' menu
    options_menu = tk.Menu(menubar, tearoff=0)
    options_menu.add_command(label="Change Window Colors", command=update_window_color)
    options_menu.add_command(label="Reset Window Colors", command=reset_window_color)
